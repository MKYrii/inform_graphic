...
def main_window():
    ...
    def small_manu(root):
        ...
        filter_menu.add_command(label='Sharpness', command=lambda : apply_filter(ImageFilter.SHARPEN))
        ...
        # функции изменения света и тени
        lighting_menu = Menu(menuu_bar, tearoff=0)
        light_menu = Menu(lighting_menu, tearoff=0)
        light_menu.add_command(label='-3', command=lambda: light(-50))
        light_menu.add_command(label='-2', command=lambda: light(-25))
        light_menu.add_command(label='-1', command=lambda: light(-10))
        light_menu.add_command(label='+1', command=lambda: light(10))
        light_menu.add_command(label='+2', command=lambda: light(25))
        light_menu.add_command(label='+3', command=lambda: light(50))

        shadow_menu = Menu(lighting_menu, tearoff=0)
        shadow_menu.add_command(label='-3', command=lambda: shadow(-50))
        shadow_menu.add_command(label='-2', command=lambda: shadow(-25))
        shadow_menu.add_command(label='-1', command=lambda: shadow(-10))
        shadow_menu.add_command(label='+1', command=lambda: shadow(10))
        shadow_menu.add_command(label='+2', command=lambda: shadow(25))
        shadow_menu.add_command(label='+3', command=lambda: shadow(50))

        # функция изменения контрастности
        contrast_menu = Menu(menuu_bar, tearoff=0)
        contrast_menu.add_command(label='-4', command=lambda: contrast(-50))
        contrast_menu.add_command(label='-3', command=lambda: contrast(-35))
        contrast_menu.add_command(label='-2', command=lambda: contrast(-20))
        contrast_menu.add_command(label='-1', command=lambda: contrast(-5))
        contrast_menu.add_command(label='+1', command=lambda: contrast(5))
        contrast_menu.add_command(label='+2', command=lambda: contrast(20))
        contrast_menu.add_command(label='+3', command=lambda: contrast(35))
        contrast_menu.add_command(label='+4', command=lambda: contrast(50))

        lighting_menu.add_cascade(label='Light', menu=light_menu)
        lighting_menu.add_cascade(label='Shadows', menu=shadow_menu)
    ...
    def light(x):
        ...
        w = image.size[0]  # Определяем ширину
        h = image.size[1]  # Определяем высоту
        pix = image.load()
        for i in range(w):
            for j in range(h):
                a = pix[i, j][0] + x
    ...
    def shadow(x):
        '''
        Изменение теней (сильнее затрагивает темные участки)
        '''
        current_tab, path, image = get_current_data()
        if not current_tab:
            return
        ...
        for i in range(w):
            for j in range(h):
                a = pix[i, j][0] - int(ceil(x * (abs(pix[i, j][0] - 255) / 255)))
                b = pix[i, j][1] - int(ceil(x * (abs(pix[i, j][0] - 255) / 255)))
                c = pix[i, j][2] - int(ceil(x * (abs(pix[i, j][0] - 255) / 255)))
                if a < 0:
                    a = 0
                if b < 0:
                    b = 0
    ...
    def contrast(x):
        ...
        for i in range(0, w, 8):
            for j in range(0, h, 8):
                ...
                b = pix[i, j][1]
                c = pix[i, j][2]
                av_color += a * 0.299 + b * 0.587 + c * 0.114
                av_color_n += 1
        k = av_color / av_color_n
        for i in range(w):
            for j in range(h):
                a = pix[i, j][0] + int(ceil(x * ((pix[i, j][0] - k) / 255)))
                b = pix[i, j][1] + int(ceil(x * ((pix[i, j][1] - k) / 255)))
                c = pix[i, j][2] + int(ceil(x * ((pix[i, j][2] - k) / 255)))
